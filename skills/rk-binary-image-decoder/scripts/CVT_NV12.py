#!/usr/bin/env python3
"""
CVT_NV12: Convert NV12 (YUV 4:2:0 8-bit) format to PNG
...
def nv12_to_yuv_planes(file_data: bytes, width: int, height: int, stride: int | None = None) -> tuple:
    """
    Convert NV12 data to separate Y, U, V planes.

    Args:
        file_data: Raw binary NV12 data
    ...
    # Extract UV plane (subsampled 2x2)
    ...
    # UV is subsampled: each 2x2 block shares the same U/V
    for y in range(height):
        uv_row_offset = (y // 2) * stride
...
def main():
    if len(sys.argv) not in (5, 6):
        print(f"Usage: {sys.argv[0]} input.nv12 width height [stride] output.png", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        Y, U, V = nv12_to_yuv_planes(file_data, width, height, stride)
    except ValueError as e:
        print(f"Failed to read NV12 file: {e}", file=sys.stderr)
        sys.exit(2)
